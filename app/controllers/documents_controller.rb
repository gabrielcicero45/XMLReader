  ...
  def export
    ...
    workbook.add_worksheet(name: "Relatório do Documento") do |sheet|
      ...
      @document.products.each do |product|
        sheet.add_row [product['nome'], product['ncm'], product['cfop'], product['unidade'], product['quantidade'], product['valor_unitario']]
      end

      sheet.add_row ["Impostos Associados"]
      sheet.add_row ["ICMS:", @document.taxes.present? && @document.taxes['icms'].present? ? number_to_currency(@document.taxes['icms'].to_f) : 'Não disponível']
      sheet.add_row ["IPI:", @document.taxes.present? && @document.taxes['ipi'].present? ? number_to_currency(@document.taxes['ipi'].to_f) : 'Não disponível']
      sheet.add_row ["PIS:", @document.taxes.present? && @document.taxes['pis'].present? ? number_to_currency(@document.taxes['pis'].to_f) : 'Não disponível']
